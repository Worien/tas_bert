		...
		if tokenize_method == "word_split":
			...
			for (i, token) in enumerate(tokens_a):
				if token.startswith('##'):
					...
					word_num += 1
					print("ner_labels_org len", len(ner_labels_org))
					print("ner_labels_org ", ner_labels_org)
					print("i", i)
					print("token_bias_num", token_bias_num)
					print("tokens_a", tokens_a)
					print("tokens_a len", len(tokens_a))
					index = i - token_bias_num
					if len(ner_labels_org) <= index:
						ner_labels_a.append(ner_labels_org[index-1])
					else:
						ner_labels_a.append(ner_labels_org[index])
